        ...
        switch (choice) {
            ...
            if (mPlayerSet.size() > 0) {
              if (mTeams.getTeams().size() > 0) {
                ...
                if (mCurrentTeam.getPlayerCount() < Team.MAX_NUM_PLAYERS) {
                  ...
                  if (mCurrentTeam.getPlayers().size() > 1) {
                    Prompter.mCounter--;
                  }
          ...
          case "remove":
            if (mTeams.getTeams().size() > 0) {
              ...
              setCurrentTeam(teamChosen);
              if (mCurrentTeam.getPlayers().size() > 0) {
            ...
            if (mTeams.getTeams().size() > 0) {
              ...
              if (reportType.equalsIgnoreCase("height")) {
                ...
                if (mCurrentTeam.getPlayers().size() > 0) {
                  ...
                  for (int i = Player.MIN_HEIGHT; i <= Player.MAX_HEIGHT; i = i + 4) {
                ...
                for (Map.Entry<String, Map<String, Integer>> entry : mapTeamToExpCounts.entrySet()) {
                  ...
                  double dExpPercent = (dExpCount / (dExpCount + dInExpCount)) * 100;
                  double dInExpPercent = (dInExpCount / (dExpCount + dInExpCount)) * 100;
                  // Calculates the percentage of experienced players on each team.
                  String strExpPercent = String.valueOf(dExpPercent) + "%";
                  String strInExpPercent = String.valueOf(dInExpPercent) + "%";
                  System.out.printf("\t%-20s: %d (%s)%n", expKey, expCount, strExpPercent);
                  System.out.printf("\t%-20s: %d (%s)%n", inExpKey, inExpCount, strInExpPercent);
                }
